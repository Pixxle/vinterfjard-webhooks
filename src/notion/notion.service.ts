...
export class NotionService {
  ...
  private convert_telegram_message_to_notion_entries(
    ...
    const filtered = telegram_message.message.text
      .split("\n")
  ...
  public async async_add_to_database(telegram_message: TelegramMessage) {
    const entries =
      this.convert_telegram_message_to_notion_entries(telegram_message);
    if (entries.length === 0) {
